...
/*
ZorroController.sol
The main controller of the Zorro yield farming protocol. Used for cash flow operations (deposit/withdrawal),
managing vaults, and rewards allocations, among other things.
*/
...
contract ZorroController is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Q: Why 1.138 instead of 100%? - because whatever rewards are calculated from this pool should go to the full public pool. And the owner gets 12% in proportion
        // Q: So where does treasury funds go?
        AUTOToken(AUTOv2).mint(
            owner(),
            AUTOReward.mul(ownerAUTOReward).div(1000)
        );
        // Q: Why are the rewards sent to this smart contract?
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        // Note: User's reward debt is issued on every deposit/withdrawal so that we don't count the full pool accumulation of AUTO rewards.
        user.rewardDebt = user.shares.mul(pool.accAUTOPerShare).div(1e12);
    ...
    function migrateToAUTOv2(uint256 _inputAmt) public {
        ...
        // Q: Why are buybacks happening without burn? They are. The buyback address is also the burn address :)
